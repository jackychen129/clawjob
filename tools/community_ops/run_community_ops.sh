...
# 6) 无人接取任务提醒（需 CLAWJOB_ACCESS_TOKEN 或 ADMIN 登录 token）
send_unpicked_reminders() {
  ...
  if python3 "$ROOT_DIR/tools/send_unpicked_reminders.py" "${extra_args[@]}" >> "$LOG_DIR/unpicked_reminders.log" 2>&1; then
...
# 5) 可选：内部 probe token 触发 register-via-skill 探活（仅当显式开启）
...
# 6) 可选：无人接取任务 24h 提醒（需 CLAWJOB_ACCESS_TOKEN 或 ADMIN 凭据）
send_unpicked_reminders() {
  local token="${CLAWJOB_ACCESS_TOKEN:-}"
  if [[ -z "$token" && -n "${ADMIN_USERNAME:-}" && -n "${ADMIN_PASSWORD:-}" ]]; then
    token="$(curl -fsS --max-time 20 -X POST "$API_URL/auth/login" \
  ...
  CLAWJOB_API_URL="$API_URL" CLAWJOB_ACCESS_TOKEN="$token" \
    python3 "$ROOT_DIR/tools/send_unpicked_reminders.py" >> "$LOG_DIR/unpicked_reminders.log" 2>&1 || {
    log "WARN unpicked-reminders failed (see $LOG_DIR/unpicked_reminders.log)"
    return 0
  }
  log "unpicked-reminders OK -> $LOG_DIR/unpicked_reminders.log"
