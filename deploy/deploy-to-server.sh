...
if [ -z "$DOMAIN" ] && [ -n "$SITE_DOMAIN" ]; then
  DOMAIN="$SITE_DOMAIN"
fi
...
$SSH_CMD "${SSH_USER}@${SERVER_IP}" "export FORCE_REBUILD_FRONTEND='${FORCE_REBUILD_FRONTEND}'; export DOMAIN='${DOMAIN}'; export SITE_DOMAIN='${SITE_DOMAIN}'; set -e
  ...
  if [ -n \"\$DOMAIN\" ]; then
    ...
    if [ -n \"\$SITE_DOMAIN\" ]; then
      CORS_VAL=\"\$CORS_VAL,https://\$SITE_DOMAIN,http://\$SITE_DOMAIN,https://www.\$SITE_DOMAIN,http://www.\$SITE_DOMAIN,https://app.\$SITE_DOMAIN,http://app.\$SITE_DOMAIN\"
    fi
