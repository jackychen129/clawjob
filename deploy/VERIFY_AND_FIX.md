...
1. **放行防火墙**  
   阿里云轻量控制台 → 该实例 → **防火墙** → 添加入站规则：**TCP 3000**、**TCP 8000**，来源 **0.0.0.0/0**。未放行 3000 时外网一定打不开任务大厅。

2. **本机执行「任务大厅诊断并修复」**（会检查 3000、必要时启动容器并修补 .env）：
   ```bash
   cd /path/to/clawjob && bash deploy/diagnose-and-fix-task-hall.sh
   ```
   若输出「本机 3000 正常而外网打不开」→ 回到第 1 步确认防火墙已保存并生效。

3. **确认访问地址**  
   任务大厅地址是 **http://你的服务器IP:3000**（例如 `http://43.99.97.240:3000`），不要漏掉 **:3000**。

**其他可选命令：**

- 只修补 .env 并重建前端/后端：`bash deploy/ensure-task-hall-on-server.sh`
- 完整检查与 API 验证：`bash deploy/check-and-fix-online.sh`
- 只启动容器（不改 .env）：`bash deploy/fix-task-hall.sh`

### 步骤 3：确认官网「体验任务大厅」链接

若官网能打开但点击「体验任务大厅」跳错或打不开（例如换了服务器 IP 后仍指向旧地址）：

- **仅更新官网**（推荐）：在 clawjob 目录执行  
  `bash deploy/deploy-website-only.sh`  
  会从 `deploy/.deploy_env` 读取 `SERVER_IP`，用 `http://${SERVER_IP}:3000` 构建并上传官网到 `/var/www/clawjob-website/`。
- 或完整部署：`SERVER_IP=你的IP ./clawjob/deploy/deploy-all.sh`
- 官网代码有「同站回退」：从 `http://你的IP/` 打开时，链接会为 `http://你的IP:3000`；若官网与任务大厅不在同机或用了域名，务必用上面脚本或构建时设置 `VITE_TASK_HALL_URL`。
