...
export SSH_USER
# 公网域名（链接与前端构建均使用域名，不用 IP）
export SITE_DOMAIN="${SITE_DOMAIN:-clawjob.com.cn}"
export SSL_DOMAIN="${SSL_DOMAIN:-$SITE_DOMAIN}"
...
if [ -z "$WEBSITE_ROOT" ] || [ ! -d "$WEBSITE_ROOT" ]; then
  ...
  if [ -n "$VITE_TASK_HALL_URL" ]; then
    ...
    export VITE_TASK_HALL_URL="https://app.${SITE_DOMAIN}"
  elif [ -n "$SSL_DOMAIN" ]; then
    export VITE_TASK_HALL_URL="https://app.${SSL_DOMAIN}"
  elif [ -n "${SITE_DOMAIN:-}" ]; then
    export VITE_TASK_HALL_URL="https://app.${SITE_DOMAIN}"
  else
    export VITE_TASK_HALL_URL="https://app.clawjob.com.cn"
  fi
  if [ -z "$VITE_STATS_API_URL" ] && [ -n "$SSL_DOMAIN" ]; then
    export VITE_STATS_API_URL="https://api.${SSL_DOMAIN}"
  fi
  # 与仅 IP 部署并存：在 .deploy_env 设置 SITE_DOMAIN=clawjob.com.cn 时显式指定统计 API，避免官网 bundle 仍指向 http://IP:8000
  if [ -z "$VITE_STATS_API_URL" ] && [ -n "${SITE_DOMAIN:-}" ]; then
    export VITE_STATS_API_URL="https://api.${SITE_DOMAIN}"
  fi
  if [ -z "$VITE_STATS_API_URL" ]; then
    export VITE_STATS_API_URL="https://api.clawjob.com.cn"
  fi
...
echo "  可选：RUN_SEED_DEMO=1 填充演示数据；FORCE_REBUILD_FRONTEND=1 强制重建前端；FORCE_REBUILD_BACKEND=1 重建后端（慎用，易触发 torch 重装）；DEPLOY_WAIT_SECONDS=30 延长等待"
