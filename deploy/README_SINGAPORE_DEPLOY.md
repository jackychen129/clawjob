...
### 方式 A：一键创建 ECS + 部署官网与 ClawJob + 验证（推荐）

在 **jasonproject** 目录下（与 clawjob、clawjob-website 同级）执行：

```bash
# 配置阿里云 AccessKey（二选一）
export ALIBABA_CLOUD_ACCESS_KEY_ID=你的AccessKeyId
export ALIBABA_CLOUD_ACCESS_KEY_SECRET=你的AccessKeySecret
# 或将两行 accessKeyId xxx / accessKeySecret xxx 放入 父目录/aliyun/accesskey.txt

# 本机密钥默认 ~/Downloads/newclawjobkey.pem，否则：
# export LOCAL_SSH_KEY_PATH=~/.ssh/你的密钥.pem

cd /path/to/jasonproject
bash clawjob/deploy/create-ecs-singapore-and-deploy.sh
```

脚本会：在新加坡创建 **ecs.e-c1m2.large**、将你的公钥导入并绑定实例（可直接用本机私钥 SSH）、放行 22/80/443/3000/8000、安装 Docker 与 Nginx、部署官网与 ClawJob、最后跑功能验证。

### 方式 B：用阿里云 CLI 创建新加坡 ECS（本机密钥）

已安装并配置 [阿里云 CLI](https://help.aliyun.com/zh/cli/) 后：

```bash
cd /path/to/clawjob
# 配置：aliyun configure（填 AccessKeyId、AccessKeySecret，Region 可填 ap-southeast-1）
bash deploy/create-ecs-singapore-cli.sh
```

脚本会：导入本机公钥（默认 `~/Downloads/newclawjobkey.pem`）、创建 **ecs.e-c1m2.large**、放行 22/80/443/3000/8000，输出 `PUBLIC_IP` 与 SSH 命令。创建完成后可按「方式 A」从步骤 2 起部署（或执行 `deploy-all.sh`）。

### 方式 C：仅用 Python 脚本创建（使用本机密钥）
...
# 可选：export LOCAL_SSH_KEY_PATH=~/Downloads/newclawjobkey.pem
python3 deploy/aliyun_ecs_create.py
```

脚本默认地域 **ap-southeast-1（新加坡）**、规格 **ecs.e-c1m2.large**。若本机存在 `~/Downloads/newclawjobkey.pem`，会导入公钥并绑定实例（输出无 PASSWORD）；否则需设置 `ALIBABA_CLOUD_ECS_PASSWORD` 或脚本会生成密码。输出含 `INSTANCE_ID`、`PUBLIC_IP`。**注意**：`InvalidAccountStatus.NotEnoughBalance` 需先在阿里云控制台充值。
...
## 二、部署前/后：运行数据库迁移（若已有库）

若服务器上已存在 ClawJob 数据库，更新代码后需执行新增字段迁移：
...
with engine.connect() as c:
    ...
    c.execute(text(\"ALTER TABLE tasks ADD COLUMN IF NOT EXISTS creator_agent_id INTEGER REFERENCES agents(id)\"))
    c.commit()
print(\"Migrations done\")
"'
```

首次全新部署可跳过（表由 init_db 创建时已含这些列）。
