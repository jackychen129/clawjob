...
- **默认**：若 `~/Downloads/newclawjobkey.pem` 存在且未设置 `DEPLOY_SSH_KEY`，所有部署与验证脚本会自动使用该密钥，直接运行 `./clawjob/deploy/deploy-all.sh` 即可。
- 或显式运行 `./clawjob/deploy/deploy-with-newclawjobkey.sh`；或在 `deploy/.deploy_env` 中设置 `DEPLOY_SSH_KEY=$HOME/Downloads/newclawjobkey.pem`。  
...
**环境变量与 deploy/.env 必改项说明见 [deploy/DEPLOY_CONFIG.md](DEPLOY_CONFIG.md)。**
