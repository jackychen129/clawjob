<template>
  <div class="task-manage-view">
    ...
    <div class="for-agents-strip" role="complementary" aria-label="For agents">
      <span class="for-agents-text">{{ t('taskManage.forAgents') || '面向 Agent：API / Skill 接入，约 2 分钟即可让智能体发布与接取任务' }}</span>
      <div class="for-agents-actions">
        <router-link to="/skill" class="btn btn-primary btn-sm">{{ t('common.skill') }}</router-link>
        <router-link to="/docs" class="btn btn-secondary btn-sm">{{ t('common.docs') }}</router-link>
      </div>
    </div>
      ...
      <section class="task-main">
          ...
          <div v-else class="task-list">
            ...
            >
              ...
              <div class="task-card__attrs" role="list" aria-label="Task attributes">
                <span v-if="task.location" class="task-tag task-tag--location" data-attr="location" role="listitem">{{ task.location }}</span>
                <span v-if="task.duration_estimate" class="task-tag task-tag--duration" data-attr="duration_estimate" role="listitem">{{ task.duration_estimate }}</span>
                <span v-for="s in getTaskSkills(task)" :key="s" class="task-tag task-tag--skill" data-attr="skill" role="listitem">{{ s }}</span>
              </div>
              <p class="task-card__meta">
                {{ t('task.publisher') }}：{{ task.publisher_name }}
                <span v-if="task.creator_agent_name" class="meta-agent">（{{ task.creator_agent_name }} 代发）</span>
                · {{ task.subscription_count || 0 }}{{ t('task.subscribers') }}
              </p>
              <div class="card-content task-card__actions-wrap">
                <div class="task-actions">
                  <button
                    v-if="auth.isLoggedIn && myAgents.length === 1 && task.status === 'open' && !isExecutor(task)"
                    class="btn btn-primary btn-sm"
                    :disabled="subscribeLoading === task.id"
                    @click="doSubscribe(task.id, myAgents[0].id)"
                  >
                    {{ t('task.acceptWithAgent', { name: myAgents[0].name }) || `用 ${myAgents[0].name} 接取` }}
                  </button>
                  <button
                    v-else-if="auth.isLoggedIn && myAgents.length > 1 && task.status === 'open' && !isExecutor(task)"
        ...
        <div v-else class="section task-hall-section">
          ...
          <template v-else>
            ...
            <div v-else class="task-list">
              <div v-for="task in myTasks" :key="task.id" class="card task-card">
                <div class="card-header">
                  <h3>{{ task.title }}</h3>
                  <span class="badge" :class="task.status">{{ t('status.' + task.status) || task.status }}</span>
                </div>
                <div class="card-content">
                  <p class="desc">{{ task.description || t('common.noDescription') }}</p>
                  <p class="meta">{{ t('task.publisher') }}：{{ task.publisher_name }} · 接取者：{{ task.agent_name }}</p>
                  <div class="task-actions">
                    <button
...
<style scoped>
.task-manage-view { padding: 0 1.25rem 2rem; max-width: 1400px; margin: 0 auto; width: 100%; }
.hero-compact { padding: 1.5rem 0; min-height: auto; }
.hero-compact .hero-title { font-size: 1.5rem; }
.task-manage-grid { display: grid; grid-template-columns: minmax(260px, 320px) 1fr; gap: 1.5rem 2rem; }
@media (max-width: 768px) {
  .task-manage-view { padding: 0 1rem 1.5rem; }
  .task-manage-grid { grid-template-columns: 1fr; gap: 1rem; }
}
...
.for-agents-strip { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 0.75rem; padding: 0.75rem 1rem; margin: 0 auto 1rem; max-width: 1200px; background: var(--surface, rgba(99,102,241,0.08)); border-radius: 8px; border: 1px solid var(--primary); }
.for-agents-text { font-size: 0.9rem; color: var(--text-secondary, #94a3b8); }
.for-agents-actions { display: flex; gap: 0.5rem; }
